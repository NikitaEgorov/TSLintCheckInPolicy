//--------------------------------------------------------------------------
// <copyright file="EvaluationProcess.cs" company="Ralph Jansen">
...
namespace RalphJansen.StyleCopCheckInPolicy.Policy
{
    ...
    internal sealed class EvaluationProcess : IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="RalphJansen.StyleCopCheckInPolicy.Policy.EvaluationProcess"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Finalizes an instance of the <see cref="RalphJansen.StyleCopCheckInPolicy.Policy.EvaluationProcess"/> class.
        /// </summary>
        ~EvaluationProcess()
        {
            this.Dispose(false);
        ...
        private Collection<FileInfo> GetFilesToAnalyze()
        {
            ...
            if (this.Context.PendingCheckin.PendingChanges == null
                || this.Context.PendingCheckin.PendingChanges.CheckedPendingChanges == null
                || this.Context.PendingCheckin.PendingChanges.CheckedPendingChanges.Length <= 0)
            {
                return files;
            }
            ...
            {
                var isFile = pendingChange.ItemType != ItemType.File;
                var isCsFile = string.Compare(Path.GetExtension(pendingChange.FileName), ".cs", true, CultureInfo.CurrentCulture) != 0;
                var isChangeIgnored = this.IsChangeIgnored(pendingChange);

                if (isFile || isCsFile || isChangeIgnored)
                {
        ...
        private Dictionary<string, Collection<FileInfo>> GenerateProjectFileMap()
        {
            ...
            foreach (FileInfo file in this.GetFilesToAnalyze())
            {
                ...
                {
                    // Add the DummyDir path to the current file because there isn't any search for the current settings path. There is only the ParentSettingsPath.
                    string projectSettingsPath = this.console.Core.Environment.GetParentSettingsPath(file.DirectoryName + "\\DummyDir\\");
        ...
        private IList<CodeProject> GetProjectsToAnalyze()
        {
            ...
            {
                CodeProject project = new CodeProject(pair.Key.GetHashCode(), pair.Key, new StyleCop.Configuration(new string[0]));

//--------------------------------------------------------------------------
// <copyright file="TfsCheckInPolicyEnvironment.cs" company="Ralph Jansen">
...
namespace RalphJansen.StyleCopCheckInPolicy.Policy
{
    ...
    internal sealed class TfsCheckInPolicyEnvironment : FileBasedEnvironment
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="RalphJansen.StyleCopCheckInPolicy.Policy.TfsCheckInPolicyEnvironment"/> class.
        /// </summary>
        ...
        public override Settings GetProjectSettings(CodeProject project, bool merge, out Exception exception)
        {
            if (project == null)
            {
                ThrowHelper.ThrowArgumentNullException("project");
            }
            ...
            if (!string.IsNullOrEmpty(project.Location))
            {
                ...
                string path = Path.Combine(Path.GetDirectoryName(project.Location), Settings.DefaultFileName);
                if (!File.Exists(path))
                {
                    // Attempt to find the alternate file in the current folder.
                    path = Path.Combine(Path.GetDirectoryName(project.Location), Settings.AlternateFileName);
